...
static void button_handler_execute(app_button_cfg_t * p_btn, uint32_t transition)
{
    if (m_evt_schedule_func != 0)
    {
        ...
        do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 40, (uint8_t*) "..\\lib\\nrf51822\\Source\\app_common\\app_button.c"); } while (0); } } while (0);
    }
    else
    {
        if(transition == 1)
