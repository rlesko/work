...
#line 6 "..\\src\\zes_sns.c"
#line 7 "..\\src\\zes_sns.c"
...
zes_sns_read(zes_sns_t *s, uint8_t a, uint8_t l, uint8_t *b)
{
	...
	res_t r = res_Ok;
	if (r == res_Ok)
		r = s->bus->trx(s->bus, 1, &a, 0, zes_bus_Tx);
	if (r == res_Ok)
		r = s->bus->trx(s->bus, l, 0, b, zes_bus_Rx|zes_bus_Stop);
...
zes_sns_write(zes_sns_t *s, uint8_t a, uint8_t l, const uint8_t *b)
{
	...
	if (r == res_Ok)
		r = s->bus->trx(s->bus, 1, &a, 0, zes_bus_Tx);
	if (r == res_Ok)
		r = s->bus->trx(s->bus, l, b, 0, zes_bus_Tx|zes_bus_Stop);
